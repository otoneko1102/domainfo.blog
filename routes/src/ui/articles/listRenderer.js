      ...
      articles.forEach((article) => {
        ...
        articlesHTML += `
          <div class="article-card">
            ...
            <a href="${link}" class="article-title"><h2>${article.title}</h2></a>
            <div class="article-meta">
              <div class="article-dates">
                <span>投稿日: <time datetime="${article.createdAt}">${createdDate}</time></span>
                ${isUpdated ? `<span>最終更新日: <time datetime="${article.updatedAt}">${updatedDate}</time></span>` : ""}
              </div>
              <div class="article-meta-right">
                ${mode === "admin" ? `<span class="status ${article.public ? "public" : "private"}">${article.public ? "公開" : "非公開"}</span>` : ""}
                ${mode === "admin" ? `<button class="article-delete-btn" data-id="${article.id}" data-title="${article.title}">削除</button>` : ""}
              </div>
            </div>
          </div>`;
      });
